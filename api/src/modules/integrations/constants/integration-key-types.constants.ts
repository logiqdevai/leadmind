...
> = {
  ...
  // The webhook secret only exists once the user has pasted our webhook URL into
  // Scrapio and created the endpoint there, so it can't be required up front.
  [ExternalIntegrationProvider.SCRAPIO]: [IntegrationKeyType.WEBHOOK_SECRET],
};
...
export function requiredKeyTypesForProvider(
  ...
  return PROVIDER_KEY_TYPES[provider].filter(
    (key_type) => !optional.has(key_type),
  );
}
...
export const DEFAULT_ACCOUNT_SELECTION_PROVIDERS: ExternalIntegrationProvider[] =
  [ExternalIntegrationProvider.RESEND, ExternalIntegrationProvider.SMTP];
...
export function listDistinctIntegrationAccounts(
  keys: { account: string }[],
): string[] {
  return [...new Set(keys.map((key) => key.account.trim()))].sort(
