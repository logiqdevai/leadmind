...
export class CampaignMessageSendService {
  ...
  async sendByMcc(
    ...
    if (this.isTerminal(mcc.status)) {
      ...
      return {
        status: 'noop',
        reason: `MCC already in terminal state ${mcc.status}`,
      };
    }
    ...
    if (
      ...
      !hasUsableContactEmail(mcc.contact.email)
    ) {
      this.logger.warn(
    ...
    if (
      mcc.channel === Channel.EMAIL &&
      isEmailValidationBlocked(mcc.contact.email_validation_status)
    ) {
      this.logger.warn(
    ...
    if (!message) {
      const subject =
        mcc.channel === Channel.EMAIL
          ? (mcc.campaign.email_subject ?? null)
          : null;
      const rawContent =
        mcc.channel === Channel.EMAIL
    ...
    try {
      const resolvedTarget =
        ...
        (message.campaign_integration_uuid
          ? await this.resolveCampaignIntegrationTarget(
              message.campaign_integration_uuid,
            )
          : undefined);
      const providerMeta = parseEmailProviderMetadata(message.metadata);
      this.logger.log(
        `Campaign deliver mcc=${mcc.uuid} message=${message.uuid} channel=${mcc.channel} provider=${resolvedTarget?.provider ?? providerMeta?.provider ?? 'default'} account=${resolvedTarget?.account ?? providerMeta?.account ?? 'default'} to=${mcc.contact.email ?? mcc.contact.phone ?? 'unknown'}`,
      );
      ...
      await this.checkCompletion(mcc.campaign_uuid);
      this.logger.error(
        `MCC ${mcc.uuid} send failed: ${error_message}`,
  ...
  async queueDraftMessageSend(
    ...
    if (
      mcc.status === CampaignContactStatus.UNSUBSCRIBED ||
      message.contact.unsubscribed_at
    ) {
    ...
    if (senderProfileUuid) {
      await this.senderProfilesService.findOne(
        organisation_uuid,
        senderProfileUuid,
      );
      message = await this.prisma.outreachMessage.update({
  ...
  async removeCampaignOutreachMessage(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        const extraDelivered =
          row.status !== CampaignContactStatus.SENT ? 1 : 0;
        let openedCut = 0;
        let clickedCut = 0;
        let repliedCut = 0;
        if (
  ...
  private async resolveCampaignIntegrationTarget(
    campaign_integration_uuid: string,
  ): Promise<EmailProviderTarget> {
    return this.emailCredentialsService.resolveTargetByCampaignIntegrationUuid(
