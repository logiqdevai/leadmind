...
export class FollowUpDraftService {
    ...
    /** Drafts a personalized follow-up from the thread's message history, or null if AI isn't configured/available. */
    async draftFollowUp(
        ...
        try {
            const { response } = await this.aiService.generateObjectWithSchema<FollowUpDraftResult>({
                ...
                prompt: buildFollowUpDraftPrompt({
                    ...
                    contactCompany: enrollment.contact.company,
                    subject: lastSubject,
                    transcript,
                }),
                usage: {
                    operation: 'FOLLOW_UP_DRAFT',
                    reference_type: 'sequence_enrollment',
                    reference_uuid: sequence_enrollment_uuid,
                },
            });
            return response;
        } catch (error) {
            this.logger.error(
                `[draftFollowUp] AI draft failed for enrollment=${sequence_enrollment_uuid}: ${error instanceof Error ? error.message : error}`,
            );
            return null;
        }
    }
