...
interface PlaceholderInsertPopoverProps {
    ...
    triggerLabel?: string;
    onInsertHtml?: (html: string) => boolean;
}

export function PlaceholderInsertPopover({
    placement = "bottom end",
    triggerSize = "sm",
    triggerVariant = "tertiary",
    triggerLabel = "Placeholders",
    onInsertHtml,
}: PlaceholderInsertPopoverProps) {
    const [open, setOpen] = useState(false);
    const [copiedKey, setCopiedKey] = useState<string | null>(null);

    const handleSelect = async (opt: (typeof PLACEHOLDER_OPTIONS)[number]) => {
        const token = opt.copyValue ?? `{{${opt.key}}}`;
        if (opt.copyValue && onInsertHtml?.(token)) {
            setCopiedKey(opt.key);
            setOpen(false);
            toast({ title: "Inserted logo image", variant: "success", duration: 1500 });
            window.setTimeout(() => setCopiedKey((cur) => (cur === opt.key ? null : cur)), 1200);
            return;
        }
        try {
            ...
            setCopiedKey(opt.key);
            toast({
                title: `Copied ${opt.copyValue ? "logo image tag" : token}`,
                variant: "success",
                duration: 1500,
            });
            window.setTimeout(() => setCopiedKey((cur) => (cur === opt.key ? null : cur)), 1200);
                ...
                <Popover.Dialog className="outline-none">
                    <div className="px-3 pt-2.5 pb-2 border-b border-border/60">
                        ...
                        <p className="text-[11px] text-muted leading-snug mt-0.5">
                            Click to {onInsertHtml ? "insert or " : ""}copy. Paste into the subject or
                            message.
                        </p>
                    </div>
                                ...
                                <li key={opt.key}>
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            "hover:bg-surface-secondary/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent",
                                    ...
                                    >
                                        ...
                                        {isCopied ? (
                                            ...
                                            <Copy className="size-3.5 text-muted shrink-0" />
                                        )}
                                    </button>
                                </li>
                            );
