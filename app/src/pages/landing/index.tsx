...
export default function LandingPage() {
  ...
  return (
    <div className={`lf-root h-full overflow-y-auto overflow-x-hidden${isDark ? "" : " lf-light"}`} style={{ backgroundColor: "var(--lf-bg)", fontFamily: "'DM Sans', system-ui, sans-serif", color: "var(--lf-tx)" }}>
      ...
      <div className="fixed pointer-events-none z-0" style={{ bottom: "-300px", right: "-200px", width: "900px", height: "900px", background: `radial-gradient(ellipse, var(--lf-vio-glow) 0%, transparent 65%)` }} />
    ...
    <section className="max-w-6xl mx-auto px-6 pt-24 pb-28 grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-14 lg:gap-10 items-center">
      <div className="space-y-8 max-w-xl">
        ...
        <div className="flex flex-wrap gap-3">
          <Link to={isLoggedIn ? Routes.dashboard.root : Routes.auth.sign_up} className="lf-btn-primary" style={{ paddingLeft: "24px", paddingRight: "24px", paddingTop: "14px", paddingBottom: "14px" }}>
            {isLoggedIn ? "Go to Dashboard" : "Start finding leads"}
            <ArrowRight className="w-4 h-4" />
          </Link>
          {!isLoggedIn && (
            <Link to={Routes.auth.sign_in} className="lf-btn-ghost">
              Sign in
            </Link>
          )}
        </div>
    ...
    <div className="relative w-full max-w-md lf-float">
      ...
      >
        ...
        <div
          ...
          style={{
            borderColor: "var(--lf-acc-bd-sm)",
            backgroundColor: "var(--lf-inset)",
          }}
        >
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <span className="lf-pulse-dot w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: "var(--lf-grn)", borderRadius: "9999px" }} />
            <span className="truncate text-[11px]" style={{ fontFamily: "ui-monospace, Consolas, monospace", color: "var(--lf-mu)" }}>
              filter:SaaS-Founders · SF · running
            </span>
          </div>
          <span className="shrink-0 text-[11px] font-semibold" style={{ fontFamily: "ui-monospace, Consolas, monospace", color: "var(--lf-acc)" }}>
            247 leads
          </span>
        </div>
        ...
        <div className="p-3 space-y-2">
          {MOCK_LEADS.map((lead, i) => (
            <div key={lead.name} className={`lf-lead-${i + 1} flex items-center gap-3 p-2.5 rounded-xl`} style={{ backgroundColor: "var(--lf-row-bg)", border: "1px solid var(--lf-row-bd)" }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: lead.ab, color: lead.ac }}>
                {lead.initials}
              </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ backgroundColor: "var(--lf-shimmer)" }}>
              <div
                className="lf-shimmer-bar h-full rounded-full"
                style={{
                  width: "65%",
            ...
            <span className="text-xs font-bold shrink-0" style={{ color: "var(--lf-acc)" }}>
              65%
            </span>
          </div>
        </div>
      ...
      <div className="absolute -right-5 -bottom-5 lf-float-alt rounded-xl p-3 flex items-center gap-3" style={{ backgroundColor: "var(--lf-sur)", border: "1px solid var(--lf-grn-bd-lg)", boxShadow: "var(--lf-float-sh)", minWidth: "160px" }}>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "var(--lf-grn-bg)" }}>
      ...
      {/* Floating stat — AI */}
      <div className="absolute -left-5 top-14 rounded-xl p-3 flex items-center gap-3" style={{ backgroundColor: "var(--lf-sur)", border: "1px solid var(--lf-vio-bd-lg)", boxShadow: "var(--lf-float-sh)", minWidth: "155px" }}>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "var(--lf-vio-bg)" }}>
    ...
    <section id="features" className="max-w-6xl mx-auto px-6 py-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {FEATURES.map(({ icon: Icon, variant, title, description, bullets }) => {
          const cv = VARIANT_CSS[variant];
          return (
            <div key={title} className="lf-card-hover rounded-2xl p-6 space-y-5" style={{ backgroundColor: "var(--lf-sur)", border: `1px solid ${v(cv.bd)}` }}>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: v(cv.bg) }}>
                <Icon className="w-6 h-6" style={{ color: v(cv.color) }} />
              </div>
              <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "18px", color: "var(--lf-tx)" }}>{title}</h3>
  ...
  return (
    <section id="how-it-works" className="max-w-6xl mx-auto px-6 py-24">
      ...
      <div className="relative mt-20 grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">
        <div className="hidden md:block absolute top-6 left-[calc(33.33%+24px)] right-[calc(33.33%+24px)] h-px pointer-events-none" style={{ background: "linear-gradient(90deg, var(--lf-acc-bd-lg), var(--lf-vio-bd-lg))" }} />
          ...
          return (
            <div key={num} className="relative flex flex-col items-center text-center gap-4">
              <div className="relative w-full flex justify-center">
                <span className="absolute -top-6 select-none pointer-events-none" style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "88px", color: v(cv.wm), lineHeight: 1, letterSpacing: "-0.04em" }}>
                  {num}
                </span>
                <div className="relative w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: v(cv.bg), border: `1px solid ${v(cv.bdLg)}`, boxShadow: v(cv.step) }}>
                  <Icon className="w-6 h-6" style={{ color: v(cv.color) }} />
                </div>
              </div>
              <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "18px", color: "var(--lf-tx)" }}>{title}</h3>
              <p className="text-sm leading-relaxed max-w-xs" style={{ color: "var(--lf-mu)" }}>
                {body}
              </p>
            </div>
          );
...
function CTASection({ isLoggedIn }: { isLoggedIn: boolean }) {
  return (
    <section className="max-w-6xl mx-auto px-6 py-24">
      ...
      >
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% -10%, var(--lf-cta-glow) 0%, transparent 55%)" }} />

        <div className="relative space-y-8">
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, color: "var(--lf-tx)", fontSize: "clamp(28px,4vw,48px)", letterSpacing: "-0.025em", lineHeight: 1.1 }}>
          ...
          <p className="text-lg max-w-xl mx-auto" style={{ color: "var(--lf-mu)" }}>
            Join teams using {environments.APP_NAME} to discover high-fit prospects at scale and convert them with AI-personalised outreach.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link to={isLoggedIn ? Routes.dashboard.root : Routes.auth.sign_up} className="lf-btn-primary" style={{ paddingLeft: "32px", paddingRight: "32px", paddingTop: "16px", paddingBottom: "16px" }}>
  ...
  return (
    <footer className="border-t" style={{ borderColor: "var(--lf-foot-bd)" }}>
      <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-10">
            {cols.map(({ heading, links }) => (
              <div key={heading} className="space-y-3">
                ...
                {links.map((l) => (
                  <a key={l} href="#" className="lf-foot-link">
                    {l}
                  </a>
                ))}
              </div>
        ...
        <div className="mt-12 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-4" style={{ borderColor: "var(--lf-foot-bd)" }}>
          <p className="text-xs" style={{ color: "var(--lf-mu)" }}>
...
function SectionHeading({ children }: { children: ReactNode }) {
  return (
    <h2 className="text-center" style={{ fontFamily: "'Syne', system-ui, sans-serif", fontWeight: 700, fontSize: "clamp(28px,4vw,44px)", color: "var(--lf-tx)", letterSpacing: "-0.02em", lineHeight: 1.1 }}>
      {children}
    </h2>
  );
}
