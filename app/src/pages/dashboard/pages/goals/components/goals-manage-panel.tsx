...
export function GoalsManagePanel() {
    ...
    const handleSaveOne = () => {
        ...
        if (!selectedUserUuid || !Number.isFinite(count) || count < 1) return;
        upsert.mutate({
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="rounded-xl border border-border bg-surface p-4 flex flex-col gap-4">
                <div>
                    <h2 className="text-sm font-semibold text-foreground">Set member goal</h2>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                    <Select
                        ...
                        onChange={(key) => setSelectedUserUuid(String(key ?? ""))}
                    ...
                    <Select
                        aria-label="Period"
                        value={period}
                        onChange={(key) => setPeriod(String(key) as GoalPeriod)}
                    >
                        <Select.Trigger className="w-full">
                            <Select.Value />
                            <Select.Indicator />
                ...
                <div>
                    <Button
                        ...
                        isDisabled={!selectedUserUuid || upsert.isPending}
                    >
                        Save goal
                    </Button>
                </div>
            ...
            <div className="rounded-xl border border-border bg-surface p-4 flex flex-col gap-4">
                <div>
                    <h2 className="text-sm font-semibold text-foreground">Bulk apply</h2>
                    <p className="text-xs text-muted mt-0.5">
                        ...
                        {goals.map((goal) => (
                            ...
                            >
                                ...
                                <Button
                                    size="sm"
                                    variant="danger"
                                    onPress={() => deactivate.mutate(goal.uuid)}
