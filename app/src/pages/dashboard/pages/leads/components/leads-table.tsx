...
import { LeadStatus, MsgStatus } from "@/features/contacts/interfaces/contact.interface";
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        ...
        header: "Drafts",
        cell: (info) => {
          const drafts = info.row.original.outreach_messages?.filter((m) => m.status === MsgStatus.PENDING).length ?? 0;
          return (
            <Chip size="sm" variant="soft" color={drafts > 0 ? "warning" : "default"}>
              ...
              renderEmptyState={() =>
                isLoading ? null : (
                  <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-muted">
                    ...
                    <p className="text-xs">Adopt a lead from the Public Leads Directory to get started.</p>
                  </div>
                )
              }
            >
