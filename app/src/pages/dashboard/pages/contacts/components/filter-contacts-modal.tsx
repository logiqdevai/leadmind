...
import { ListPlus, Trash2, type LucideIcon } from "lucide-react";
import { ActionButtonWithPending } from "@/components/ui/action-button-with-pending";
import { useAddListContactsBelowScore } from "@/features/contact-lists/hooks/use-contact-lists";
import { useDeleteContactsBelowScore } from "@/features/contacts/hooks/use-contacts";
import { cn } from "@/lib/utils";
import { ParentListSelect } from "@/pages/dashboard/pages/lists/components/parent-list-select";
...
const ACTION_OPTIONS: ActionOption[] = [
    {
        id: FilterActions.ADD_TO_LIST,
        title: "Add to a List",
        summary: "Add low-score contacts to a list you choose.",
        icon: ListPlus,
    },
    {
        id: FilterActions.DELETE,
        title: "Delete",
        summary: "Permanently remove low-score contacts from your CRM.",
        icon: Trash2,
    },
];
...
export const FilterContactsModal: FC<FilterContactsModalProps> = ({
    ...
    const addBelowScore = useAddListContactsBelowScore();
    const deleteBelowScore = useDeleteContactsBelowScore();
    ...
    const isPending = addBelowScore.isPending || deleteBelowScore.isPending;

    const handleConfirm = async () => {
        if (action === FilterActions.ADD_TO_LIST) {
            if (!targetListUuid) return;
            await addBelowScore.mutateAsync({ listUuid: targetListUuid, min_score: minScore });
            onComplete?.();
            onOpenChange(false);
            return;
        }
        if (action === FilterActions.DELETE) {
            await deleteBelowScore.mutateAsync(minScore);
            onComplete?.();
            onOpenChange(false);
        }
    };
    ...
    const confirmDisabled =
        isPending ||
        action == null ||
        (action === FilterActions.ADD_TO_LIST && !targetListUuid);

    const confirmLabel =
        action === FilterActions.ADD_TO_LIST
            ? "Add contacts"
            : action === FilterActions.DELETE
              ? "Delete contacts"
              : "Continue";
                                            ...
                                            >
                                                <span className="relative mt-0.5 flex size-5 shrink-0 items-center justify-center">
                                                    <input
                                                        ...
                                                        onChange={() => {
                                                            setAction(option.id);
                                                            if (option.id !== FilterActions.ADD_TO_LIST) {
                                                                setTargetListUuid(null);
                                                            }
                                                        }}
                                                    ...
                                                    <span className="flex items-center gap-2">
                                                        <Icon
                                                            className={cn(
                                                                "size-4 shrink-0",
                                                                selected
                                                                    ? option.id === FilterActions.DELETE
                                                                        ? "text-danger"
                                                                        : "text-accent"
                                                                    : "text-muted",
                                                            )}
                                                ...
                                                <div className="overflow-hidden">
                                                    {selected ? (
                                                        <div className="space-y-4 border-t border-border/70 px-6 pb-5 pt-4">
                                                            ...
                                                            {option.id === FilterActions.ADD_TO_LIST ? (
                                                                <div className="space-y-3">
                                                                    <p className="text-xs leading-relaxed text-muted">
                                                                        Contacts scoring under{" "}
                                                                        {minScore} are added to the
                                                                        list below. They stay in your
                                                                        CRM and any other lists.
                                                                    </p>
                                                                    <ParentListSelect
                                                            ...
                                                            {option.id === FilterActions.DELETE ? (
                                                                <p className="text-xs leading-relaxed text-muted">
                                                                    Contacts scoring under{" "}
                                                                    {minScore} are permanently
                                                                    deleted from your CRM. This
                                                                    cannot be undone.
                                                                </p>
                                                            ) : null}
                                                            ...
                                                            <ActionButtonWithPending
                                                                ...
                                                                isPending={
                                                                    option.id === FilterActions.ADD_TO_LIST
                                                                        ? addBelowScore.isPending
                                                                        : deleteBelowScore.isPending
                                                                }
