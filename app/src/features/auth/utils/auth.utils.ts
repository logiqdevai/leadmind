...
export const formatAuthUser = (data: any): LoggedInUser => {
    return {
        ...
        organisation_uuid: data?.user?.organisation_uuid ?? null,
        organisation_role: data?.user?.organisation_role ?? null,
        organisation_name: data?.user?.organisation_name ?? null,
    };
};