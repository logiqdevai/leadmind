...
/**
 * Full conversation body for a thread - the sent-message + delivery/open/click/reply timeline
 * plus an inline reply composer. Rendered both inside a Modal (contact detail's "view
 * conversation" action) and inline in the send-history inbox view.
 */
                ...
                <div className="flex flex-col gap-3 pl-1">
                    {data.timeline.map((entry) =>
                        entry.kind === "outbound" ? (
                            ...
                            <ThreadEvent key={entry.interaction.uuid} interaction={entry.interaction} />
                        ),
                    )}
                </div>
            )}
